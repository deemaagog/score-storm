...
test.beforeEach(async ({ page, renderer }) => {
  page.setViewportSize({ width: 2000, height: 800 })
  await page.goto("/")
...
test("renders dots correctly", async ({ page }) => {
  const inputXmlString = fs.readFileSync(path.join(__dirname, "dots.musicxml"), "utf8")
